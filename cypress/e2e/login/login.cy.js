...
describe("Login", () => {
  ...
  it("TC01, Successful Login with Valid Credentials", () => {
    cy.get("#user-name").type(Username); //Filling out the Username;
    cy.get("#password").type(Password); //Filling out the Password;
  ...
  it("TC02, Error Message for Invalid Credentials", () => {
    cy.get("#user-name").type(fakeUsername); //Filling out the Username;
    cy.get("#password").type(Password); //Filling out the Password;
    cy.get("#login-button").click(); //Click on the Login button;
